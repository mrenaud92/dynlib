...
TEST_F(TestFunctionPtrCast, FptrCast_WhenGivenValidPointer_WillConvertToCallableFunction)
{
	auto result = dlibxx::util::fptr_cast<void,int,char>((void*)&load_me);
	EXPECT_TRUE(result);
}

TEST_F(TestFunctionPtrCast, FptrCast_WhenGivenNullPointer_WillNotConvertToCallableFunction)
{
	auto result = dlibxx::util::fptr_cast<void,int,char>(nullptr);
	EXPECT_FALSE(result);
}

TEST_F(TestFunctionPtrCast, FptrCast_WhenFunctionIsCallable_WillNotThrowException)
{
	auto result = dlibxx::util::fptr_cast<void,int,char>((void*)&load_me);
