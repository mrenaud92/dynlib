...
public:
	TestLoadLibrary()
		: Test{}
	{
		ON_CALL(dlfcn, dlopen(_,_)).WillByDefault(Return(reinterpret_cast<void*>(1)));
...
TEST_F(TestLoadLibrary, Loaded_WhenLibraryCannotBeFound_WillReturnFalse)
{
	...
	EXPECT_CALL(dlfcn, dlopen(StrEq(lib_name),_)).WillRepeatedly(Return(nullptr));
...
{
	auto const lib_name = std::string{ "test"};
	EXPECT_CALL(dlfcn, dlopen(StrEq(lib_name),_)).WillRepeatedly(Return(reinterpret_cast<void*>(1)));
...
class TestDlopenFlags : public TestLoadLibrary,
...
{
	auto const resolve_policy  = std::get<0>(GetParam());
	auto const resolve_options = std::get<1>(GetParam());

	auto const expected_flags = static_cast<int>(resolve_policy) | static_cast<int>(resolve_options);
...
std::vector<dlibxx::options> combine_resolve_options()
{
	...
	for(auto scope: {dlibxx::options::global, dlibxx::options::local})
	{
		...
		result.push_back(scope | dlibxx::options::no_delete | dlibxx::options::no_load);
		result.push_back(scope | dlibxx::options::no_delete | dlibxx::options::deep_bind);
		result.push_back(scope | dlibxx::options::no_delete | dlibxx::options::no_load | dlibxx::options::deep_bind);
		result.push_back(scope | dlibxx::options::no_load   | dlibxx::options::deep_bind);
	}
